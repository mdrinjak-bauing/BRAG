...
- **Analysis questions → analysis modes.** "Who writes about X / state of research?" → `search(mode='coverage')` (per-source split into substantial vs. peripheral; `coverage_mode='specific'` finds narrow specialist sources). "Which sub-aspects does X have?" → `search(mode='clusters')` (semantic topic map). "What do THESE sources say about X?" → `compare_positions(query, sources=[...])` side by side.
...
- Use `chunk_type="table"` when I ask for numbers or statistics, `chunk_type="figure"` for diagrams. Figure hits attach up to 3 of the **actual figure images** — look at them and read values straight off the diagram instead of guessing from the caption.
