...
def validate_gemini_key(api_key: str) -> tuple[bool, str]:
    """Lightweight live check of a Gemini API key (REST, no SDK overhead)."""
    import urllib.error
    ...
    try:
        ...
        if e.code in (400, 401, 403):
            return False, ("The key was rejected by Google. Please copy it "
                           "again from https://aistudio.google.com/apikey")
        return False, f"Google answered with an error (HTTP {e.code}). Try again."
    except OSError:
        return False, ("Could not reach Google — check your internet "
