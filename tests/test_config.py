...
def test_defaults_outside_any_context(monkeypatch):
    ...
    assert config.SOURCES_DIR == Path("/vault/sources")
    assert config.NOTES_DIR == Path("/vault/notes")
    assert config.WIKI_DIR == Path("/vault/wiki")
    assert config.DATA_DIR == Path("/vault/.brag")
    assert config.INGEST_LOG == Path("/vault/.brag/ingest_log.jsonl")
...
def test_project_context_scopes_and_resets(monkeypatch):
    monkeypatch.setattr(config, "_DEFAULT_VAULT", Path("/vault"))
    monkeypatch.setattr(config, "_DEFAULT_COLLECTION", "asb_default")
    rec = {"vault": "/projects/a/WissensWIKI", "collection": "asb_default__a"}
    with config.project_context(rec):
        assert config.VAULT == Path("/projects/a/WissensWIKI")
        assert config.SOURCES_DIR == Path("/projects/a/WissensWIKI/sources")
        assert config.NOTES_DIR == Path("/projects/a/WissensWIKI/notes")
