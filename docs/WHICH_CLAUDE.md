...
differ in **who they're for** and **how much they do on their own**. For BRAG the
...
| Surface | What it is (official framing) | Runs where | Best for | Fit for BRAG |
|---|---|---|---|---|
| **Claude (Chat)** | "Thinking, drafting and analysis with you in the loop on every step" — turn by turn | the Claude Desktop app; your MCP servers run on your machine | grounded research, writing, Q&A over your corpus | ✅ **BRAG's home** |
| **Claude Cowork** | "Give it a goal and Claude works on your computer, local files and applications to return a finished deliverable" — autonomous | a **sandboxed Linux VM inside the Claude Desktop app** (Pro/Max/Team) | multi-step file/app tasks done in the background | ⚠️ not the BRAG driver |
| **Claude Code** | "The developer agent for codebases, git and the terminal — it plans and executes, you review the diffs" | your terminal / IDE / web | building & extending the system | 🛠️ use it to **improve BRAG** |

## Why Chat is the right home for BRAG

BRAG is wired into **Claude Desktop's chat**, and that is the surface it was built
for:
...
- Claude Desktop **Projects** let you paste your `WissensWIKI/CLAUDE.md` into the
  Project's custom instructions (Claude Desktop does not read the file on its
  own), so every chat starts grounded in your field and your citation style.
...
## What Cowork is — and why it isn't the BRAG driver
...
Crucially for BRAG, Cowork **runs inside a sandboxed Linux VM** on your Mac or
...
to a host-side `docker exec` stdio server like BRAG's. Community guides for
...
**grounded, cited research chat** that BRAG provides lives in regular **Chat**.
...
review the diffs. It is the right tool to **build and extend BRAG**: add a new MCP
...
- **Chat + BRAG search:** a question sends only the **top-K retrieved passages**
...
whole task.** BRAG is built for the first.
